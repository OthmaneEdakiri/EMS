...
## ERP SaaS Platform for SMBs — V1: Inventory & Stock Management

| | |
|---|---|
| **Document Owner** | Product/Engineering |
| **Status** | Draft v2 |
| **Builds On** | MVP (v1.0) — assumed fully implemented and in pilot use |
| **Last Updated** | 2026-09-13 |
| **Tech Stack** | Laravel (PHP) · PostgreSQL · Next.js · ShadCN/ui |
| **Team assumption** | Solo developer / very small team |

**Changelog v2.0 → v2.1:** Fixed the Owner/Staff permission inconsistency around stock writes (§2, §5.1, §6, §8), added the missing oversell-policy data model column and API endpoint (§12, §13), rewrote the unedited draft language in §8, added idempotency and multi-line-aggregation rules (§8, §12), added product-type immutability once tracking starts (§8, §9), replaced an unmeasurable success metric (§3), scoped down the concurrency test requirement in the DoD (§16), added a `CHECK` constraint on `reorder_level` (§12), and converted two previously "open questions" into firm decisions (§6, §13) now that they're resolved.

---

## 0. Baseline — What Already Exists (from PRD v1 / MVP)

This document assumes the MVP is live and in use by pilot tenants, with the following already shipped and **not re-specified here**:

- Multi-tenant signup/login, Owner + Staff roles (no email invite)
- Customers CRUD (archive, not delete, when referenced)
- Products & Services CRUD, with a `type` field (`product`/`service`) that had **no behavioral effect** in MVP
...
Everything below is **new** scope layered on top of that baseline. Nothing in §0 is being redesigned — only extended.

---

## 1. Overview

### 1.1 Problem Statement (V1)
Pilot feedback on the MVP consistently surfaces one gap: businesses selling physical `product`-type items have no way to know how much stock they have left. They're back to a spreadsheet — this time to track quantity instead of payment status. The `type` field from MVP exists specifically to make this extension possible without a schema migration.

### 1.2 What "V1" Means Here
**V1 answers one additional question: can a business know, at any moment, how much of a product they have left — and does creating/sending an invoice for a product automatically reduce that number, correctly and exactly once?**

V1 does **not** attempt to answer "what should I reorder and from whom" (that's V2 — Purchasing) or "what is my inventory worth on my books" (that's V3 — Accounting). It is deliberately just: track quantity, deduct on sale, prevent overselling, allow controlled manual correction.
...
- Allow manual stock corrections (receiving initial stock, counts, damage/loss) under a single, consistent permission model — before Purchasing (V2) exists to do this properly.
...
- Purchasing / suppliers / purchase orders / goods receipt — deferred to **V2**
- Multi-warehouse / multi-location stock — deferred (single implicit "default location" per tenant in V1)
- Inventory valuation methods (FIFO/LIFO/weighted average) and accounting integration — deferred to **V3**
...
- Email/notification-based low-stock alerts (still no system email until V4 per v1 PRD)
- Per-product override of the oversell policy — V1 ships **tenant-level only** (see §6 FR-16, decided; no longer an open question)

---

## 2. Target Users & Personas (V1-relevant only)

| Persona | Role | New Need in V1 |
|---|---|---|
...
**Permission model, stated once here to avoid the v2.0 inconsistency:** any action that changes `quantity_on_hand` outside of the automatic sale/cancellation flow — enabling tracking with an opening balance, or adjusting a quantity — is **Owner-only**. Staff can trigger stock changes only indirectly, by sending an invoice (FR-11) or triggering its cancellation reversal (FR-12), both of which are system-computed, not manually entered numbers. This is the single rule applied consistently across FR-10, FR-13, and §8.
...
| Products left with negative `quantity_on_hand` **without** the tenant's oversell policy explicitly set to "warn" (i.e., an unintended negative-stock state, queryable directly from the database) | 0, in the first month |
| Time to build V1 (solo dev, focused, on top of live MVP) | 2–3 weeks |

*(The v2.0 metric "tenant confirming it was correct to block" was removed — there is no in-product feedback mechanism to collect that confirmation, and email/notifications are out of scope for V1. The replacement metric above is verifiable directly from system data instead of relying on an uncollectable signal.)*

---

## 4. V1 Scope — Feature List
...
| 3 | Enable tracking + manual stock adjustment (Owner only, single permission model) | Set opening balances, correct counts, record damage/loss — before Purchasing exists |
...
| 5 | Tenant-level oversell policy (block or warn), stored and editable | Prevents promising stock that doesn't exist, or explicitly allows it — a real, persisted setting, not just a mentioned one |
...
one-time, forward-looking starting point; V1 does not attempt to
reconstruct stock history from invoices issued before tracking
was enabled (decided — see FR-10)
        │
        ▼
...
```
*A product created fresh can enable tracking + set opening stock (default 0) in the same create form — no two-step flow required. Staff never sees this control; if a Staff user needs a product tracked, they ask the Owner, same as they already do for Team Settings in MVP.*

### 5.2 Modified Flow — Invoice → Stock Deduction (extends MVP §5.1)
```
...
```
Owner opens Tenant/Company Settings (same settings area MVP
...
tracked product for that tenant — no per-product override in V1
```
...
- **Given** a Staff user
  ...
  **Then** the "Track stock" toggle and opening-quantity field are not shown, and the underlying endpoint rejects the request (403) if called directly — this closes the v2.0 inconsistency where Staff could set an unaudited opening number while being blocked from correcting it afterward.
- **Given** a `service`-type item
  **When** anyone views its edit form
  **Then** no "Track stock" option is shown — services are never stock-tracked in V1.
- **Decision (resolved from v2.0's open question):** enabling tracking never attempts to reconstruct historical consumption from invoices issued before V1 existed. The opening balance is simply "what the Owner counts today."
...
- **Given** a `sent` invoice that had stock deducted
  **When** it is cancelled (per existing MVP cancellation rule: only from `draft` or `sent`)
...
- **Decision (resolved from v2.0's open question):** this setting is tenant-wide only in V1, with no per-product override — kept in scope with the rest of V1's minimalism; per-product override is logged in §19 as post-V1 backlog if a real need appears.
...
- **Given** a product with `track_stock = true` or any `stock_movements` history
  ...
  **Then** the change is rejected (422) server-side, with a message explaining that stock history exists and the type is locked — mirroring the existing MVP rule that `track_stock` itself cannot be disabled once history exists.

---

## 7. Non-Functional Requirements (V1-scoped, additive to MVP §7)

| Category | Requirement |
|---|---|
| **Concurrency** | Stock deduction on invoice send **must** use row-level locking (`SELECT ... FOR UPDATE` on both the invoice row and each affected product row) inside the transaction — this is the one place in the system where MVP's "last write wins" (§9/§17 of v1 PRD) is no longer acceptable, because two simultaneous sales of the last unit must not both succeed. |
...
| **Data integrity** | `quantity_on_hand` is a derived/cached value; the `stock_movements` ledger is the source of truth. A scheduled or on-demand recompute (sum of movements) must be able to detect/repair drift — implement at minimum a manual "recalculate from ledger" admin action, even if not automated in V1. |
| **Performance** | Stock check + deduction adds negligible latency to invoice-send (one indexed lookup + lock per distinct product, after aggregation); no material change to the <2s page load target from MVP. |
...
| **Localization** | All new UI strings (stock labels, adjustment reasons UI, low-stock badge, oversell-policy setting) ship in Arabic + English from day one, consistent with MVP. |

---

## 8. Business Rules (new, additive to MVP §8)
...
- Once `track_stock` is enabled on a product, it **cannot be disabled** if the product has any `stock_movements` history — this avoids silently discarding a quantity history. (Consistent with the MVP philosophy of archiving over destructive edits.)
- **Every write to `quantity_on_hand` outside the automatic sale/cancellation flow is Owner-only** — this includes both enabling tracking with an opening balance (FR-10) and later adjustments (FR-13). There is no tier of stock-write action available to Staff; this single rule replaces the v2.0 inconsistency between FR-10 and FR-13.
...
- Editing line items is already blocked once `sent` (MVP §8), so there is no "edit a sent invoice's quantities" case to reconcile against stock in V1.
...
### Reversal
- Per MVP §8, cancellation is only allowed from `draft` or `sent` — never from `partially_paid` or `paid`. As a direct consequence, stock reversal in V1 only ever applies to cancelling a `sent` (unpaid, un-deducted-from-payment) invoice. Once a payment has been recorded against an invoice, unwinding it (and any associated stock) is out of scope for V1 and remains deferred to the credit-note mechanism already planned for V3/V4 (MVP §17/§19).

### Adjustments
- Manual adjustments require a non-empty reason string — this is the only audit trail for manual changes in V1 (no approval workflow, no dual control — acceptable at pilot scale, flagged as a limitation in §17 below).
- Adjustments are Owner-only, per the unified permission rule above.

### Oversell Policy
- The oversell policy (`block` or `warn`) is a single tenant-wide setting (FR-16), applied uniformly to every tracked product for that tenant — there is no per-product override in V1.

---

## 9. Error States (new, additive to MVP §9)
...
| Staff attempts to enable stock tracking or set an opening balance | Blocked (403) — same pattern as Staff attempting Team Settings in MVP. |
...
## 10. UX Requirements (additive to MVP §10)

| Requirement | Detail |
|---|---|
| Stock visibility in invoice line picker | Available quantity shown inline next to each tracked product in the type-ahead (MVP's ShadCN Command picker), no extra click |
| Adjustment flow speed | ≤ 3 clicks from product detail to a saved adjustment |
| Low-stock badge | Reused visual language from MVP's status badges (§14 of v1 PRD) — a consistent amber/red treatment, not a new pattern |
| Oversell policy setting location | Lives in the same Tenant/Company Settings screen as MVP's currency/invoice-prefix settings — no new settings surface introduced |
| RTL | Stock list/adjustment/settings forms follow the same top-aligned label, RTL-first layout rules already established in MVP |
...
No queue, no Redis, no Horizon, no event bus, no RLS in V1 — same as MVP. Stock deduction is synchronous, inside the existing invoice-send request, using a DB transaction + row locks (see §7) rather than any async mechanism.

---

## 12. Data Model — V1 Additions

```
-- Additive columns on existing tables:
tenants (
  ...existing MVP columns...,
...
- `stock_movements` is append-only — no updates, no deletes, matching the MVP philosophy already applied to `payments` (§8 of v1 PRD: "a payment cannot be deleted once recorded").
...
- Whole integers only in V1 (no fractional quantities like 2.5 kg) — flagged as a known limitation in §17; fractional units are deferred until a real pilot need appears.

---

## 13. API Additions (extends MVP §13 conventions, same envelope/error format)
...
| GET | `/api/v1/tenant/settings` | Read tenant settings, extended in V1 to include `oversell_policy` alongside MVP's existing currency/invoice-prefix fields | Owner or Staff (read-only) |
| PATCH | `/api/v1/tenant/settings` | Update tenant settings, including `oversell_policy` (`block` \| `warn`) | Owner only |

`POST /api/v1/invoices/{id}/send` (existing MVP endpoint) is **modified**, not replaced: it now performs the lock-check-aggregate-deduct transaction described in FR-11 before committing the status change, reading `oversell_policy` from the tenant record to decide block-vs-warn. Its response envelope is unchanged; on insufficient stock it returns the standard 422 error envelope with one error entry per short product (aggregated, not per line — see §8).

`POST /api/v1/invoices/{id}/cancel` (existing MVP endpoint) is similarly modified to perform the reversal described in FR-12, guarded by the same uniqueness constraint.

*(v2.0 left the tenant-settings endpoint unspecified while the FR referenced it — this table closes that gap by extending the settings endpoint that MVP already has, rather than introducing a new one.)*

---

## 14. Design Guidelines (additions to MVP §14 stub)
...
- **Adjustment dialog** — reuse the MVP destructive-action confirmation pattern for any adjustment that reduces quantity; additive adjustments (receiving stock) don't need confirmation.
- **Oversell policy toggle** — a simple two-option control (radio or switch) in Tenant Settings, not a new page — consistent with the "stub, filled in once" philosophy of MVP §14.

---

## 15. Roadmap — Sprint Plan (Solo Developer, V1 only)

Assumes MVP is already live; these sprints are purely additive.
...
**→ V1 demoable/usable at end of Sprint 8**, without any downtime or breaking change for tenants who don't enable stock tracking.

### Post-V1 Versions (unchanged from v1 PRD, renumbered for clarity)
| Version | Scope | Trigger to start |
|---|---|---|
| **V2** | Purchasing (suppliers, POs, goods receipt — becomes the *proper* way to add stock, replacing manual "opening_balance"/"adjustment" as the main inbound path) | Pilot users need to track what they buy, not just sell |
| **V3** | Accounting (chart of accounts, journal entries, inventory valuation, P&L/Balance Sheet) | Paying customers who need books, or an accountant brought in to design it |
| **V4** | SaaS billing automation, email invites/notifications (including low-stock alerts, deferred from V1 §1.4), Dashboard, Queue/Horizon, fuller audit log, RLS, payment reversal/credit notes, fractional stock quantities | Past pilot, onboarding customers self-serve |

---

## 16. Definition of Done (DoD) — same bar as MVP §16, applied to every V1 feature
...
- [ ] **Concurrency test — minimum acceptable bar (scoped down from v2.0):** a database-level test using two overlapping transactions against the service/repository layer directly (not a full simulated HTTP race), asserting the row lock serializes the two attempts and only one succeeds. A full true-concurrency HTTP-level test is **not** required for V1 given solo-developer time constraints; if even the DB-level test proves impractical within Sprint 7, a documented manual QA script (two browser sessions, same last-unit product, both click "send" within the same second) is an acceptable substitute, with the gap noted in §17 rather than silently skipped.
...
## 17. Technical Debt & Known Limitations (V1, additive to MVP §17)
...
- No low-stock notifications (email/push) — badge is visible only when a user is actively looking at the product or stock list. Consistent with MVP's "no system email" limitation; revisit at V4.
...
- Oversell policy is tenant-wide only — a tenant that wants "block for high-value items, warn for the rest" has no way to express that in V1 (see §19 backlog).
...
- Should V1 offer per-product override of the oversell policy in a later iteration, or is tenant-wide sufficient long-term? (Tenant-wide is the firm V1 decision — see FR-16 — this question is only about whether to revisit post-V1, tracked in §19.)
...
*(The v2.0 questions about "who decides the oversell-policy scope" and "should opening balance reconstruct history" are now resolved — see FR-16 and FR-10 respectively — and have been removed from this list.)*

---

## 19. Appendix — Backlog Items Newly Identified in V1 (added to MVP §19)

- Fractional/decimal stock quantities
- Stock reservation on draft invoices (soft-hold with expiry)
- Multi-location/warehouse stock (already listed in MVP §19, reaffirmed here as still deferred)
